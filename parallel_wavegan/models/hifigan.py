...
from parallel_wavegan.layers import WNConv1d, PastFCEncoder
...
class HiFiGANGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        pre_final_scale = np.prod(upsample_scales)
        assert pre_final_scale >= final_scale
        chunks_to_omit = set()
        if pre_final_scale != final_scale:
            num_omit = pre_final_scale-final_scale
            delta = pre_final_scale/(num_omit+1)
            chunks_to_omit = set([round(delta*(i+1)) for i in range(num_omit)])
        self.chunks_to_keep = [i for i in range(pre_final_scale) if i not in chunks_to_omit]
        self.pre_final_scale = pre_final_scale
        ...
        if not extra_art:
            ...
            self.input_conv = torch.nn.Sequential([
                WNConv1d(in_channels, in_channels, kernel_size=2),
                torch.nn.Conv1d(
                    in_channels,
                    channels,
                    kernel_size,
    ...
    def forward(self, c, ar=None):
        ...
        c = self.output_conv(c) # (batch_size, 1, input_len*pre_final_scale)
        if self.pre_final_scale != self.final_scale:
            batch_size = c.shape[0]
            c = c.reshape(batch_size, 1, self.pre_final_scale, -1)
            input_len = c.shape[3]
            c = c[:,:,self.chunks_to_keep,:]
            c = c.reshape(batch_size, 1, self.final_scale*input_len)
        # print('c', c.shape)
        # exit()
