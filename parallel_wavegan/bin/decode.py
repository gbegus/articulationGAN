...
import parallel_wavegan.transforms

from parallel_wavegan.datasets import MelDataset, audio_mel_dataset
from parallel_wavegan.datasets import MelSCPDataset
from parallel_wavegan.datasets import ArtDataset
from parallel_wavegan.datasets import ArtSCPDataset, AudioSCPDataset, ArtSCPMultDataset
from parallel_wavegan.utils import load_model
from parallel_wavegan.utils import read_hdf5


def ar_loop(model, x, config, do_wsola=False, modality=None):
    '''
    Args:
        x: (art_len, num_feats)
    ...
    if config["dataset_mode"] == 'w2a':
        in_chunk_len = audio_chunk_len
        past_out_len = int(config["generator_params"]["ar_input"]/config["generator_params"]["out_channels"])
    else:
        in_chunk_len = int(audio_chunk_len/config["hop_size"])
        past_out_len = config["generator_params"]["ar_input"]
    ...
    if not do_wsola:
        ...
        if config["dataset_mode"] == 'w2a' and len(ins[-1]) < config["hop_size"]:
            ins = ins[:-1]
        prev_samples = torch.zeros((1, config["generator_params"]["out_channels"], past_out_len), dtype=x.dtype, device=x.device)
        outs = []
        ...
        extra_art = config["generator_params"]["extra_art"]
        ...
        for art_i, art in enumerate(ins): # art (in_chunk_len, num_feats)
            ...
            signal = model(art, prev_samples)  # (1, 1, audio_chunk_length)
...
def main():
    ...
    if input_transform is not None:
        input_transform = getattr(parallel_wavegan.transforms, input_transform)
    ...
    elif dataset_mode == 'a2w' or dataset_mode == 'a2w_pcd':
        if args.dumpdir is not None:
            if config["format"] == "hdf5":
                mel_query = "*.h5"
                mel_load_fn = lambda x: read_hdf5(x, "feats")  # NOQA
    ...
    with torch.no_grad(), tqdm(dataset, desc="[decode]") as pbar:
        for idx, tup in enumerate(pbar, 1):
            ...
            if dataset_mode == 'default' or dataset_mode == 'a2w' or dataset_mode == 'a2w_pcd' or dataset_mode == 'a2w_mult':
            ...
            elif dataset_mode == 'art' or dataset_mode == 'w2a':
                if "use_ar" in config["generator_params"] and config["generator_params"]["use_ar"]:
                    t0 = time.time()
                    y = ar_loop(model, c, config, do_wsola=do_wsola, modality=modality)
                    t1 = time.time()
